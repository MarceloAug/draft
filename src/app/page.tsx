...
export default function SorteioPage() {
  ...
  const init = async () => {
    ...
    const { data: existingDays } = await supabase
      .from("game_days")
      .select("id, finished, session_number")
      .eq("date", date)
    ...
    if (existingDay) {
      ...
      setSessionNumber(existingDay.session_number);
    } else {
      const { data: newDay, error: newDayErr } = await supabase
        .from("game_days")
        .insert({ date, session_number: 1 })
        .select("id, finished, session_number")
        .single();
      if (newDayErr) {
      ...
      setSessionNumber(newDay.session_number);
    }
    setGameDayId(dayId);
  ...
  async function criarNovoJogo() {
    if (!confirm("Criar um novo jogo pra hoje? Presença e sorteio começam do zero.")) return;
    setError(null);
    setBusy("day");
    try {
      const date = todayStr();
      ...
      const { data: newDay, error: newDayErr } = await supabase
        .from("game_days")
        .insert({ date, session_number: nextSession })
        .select("id, finished, session_number")
        .single();
      if (newDayErr) {
        setError(newDayErr.message);
        ...
        <div className="space-y-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={busy !== null}
              className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-orange-500 to-amber-400 px-3 py-2 text-xs font-bold text-slate-950 transition active:scale-[0.98] disabled:opacity-60"
            >
              {busy === "day" ? <Spinner /> : "➕"} Criar novo jogo
            </button>
            <button
              onClick={reabrirDia}
              disabled={busy !== null}
              className="flex items-center gap-2 rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white transition active:scale-[0.98] disabled:opacity-60"
            >
              Reabrir
            </button>
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            {sessionNumber > 1 && (
              <p className="text-xs font-medium text-orange-400">Jogo {sessionNumber} do dia</p>
            )}
          </div>
